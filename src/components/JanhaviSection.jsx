...
export const JanhaviSection = ({ data, onSelectMemory, onNavigateNext, onNavigateHome }) => {
    ...
    <div className="min-h-screen bg-slate-950 bg-scrapbook-texture text-slate-100 pt-20 pb-16 px-4 sm:px-8 overflow-x-hidden">
      <div className="max-w-5xl mx-auto space-y-16 sm:space-y-20">
        ...
        <section className="text-center pt-8 pb-4 relative">
          ...
          >
            <p className="text-amber-400 font-mono text-xs sm:text-sm uppercase tracking-widest mb-2 font-bold">
              {data.introTitle}
            </p>

            <h1 className="text-5xl sm:text-7xl font-extrabold tracking-tight bg-gradient-to-r from-amber-200 via-orange-300 to-yellow-400 bg-clip-text text-transparent font-display">
              {data.name}
            </h1>

            <p className="font-handwriting text-2xl sm:text-3xl text-amber-300/90 mt-2">
              {data.introSubtext}
            </p>
          </motion.div>
        </section>

        {/* 2. Group 1: College Days & Hangouts */}
        {data.group1Memories && (
        ...
        {/* 3. Group 2: Milestones & Adventures */}
        {data.group2Memories && (
          <section className="space-y-6">
            <div className="border-l-4 border-amber-400 pl-4 py-1">
              <h2 className="text-2xl sm:text-3xl font-bold font-display text-white">
        ...
        {/* 5. A Heartfelt Note (Clean, uncluttered emotional note) */}
        <section className="pt-4 space-y-6">
          <div className="relative rounded-3xl overflow-hidden border border-amber-400/30 shadow-2xl bg-slate-900">
            <div className="p-6 sm:p-10 bg-slate-900/90 backdrop-blur-md">
              ...
              <div className="mt-6 flex items-center justify-between border-t border-slate-800 pt-4">
                ...
                <span className="text-xs text-slate-500 font-mono">Janhavi</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-slate-800">
          ...
          <button
            onClick={() => onNavigateNext('prajakta')}
            className="w-full sm:w-auto px-6 py-3 rounded-full bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold transition-all flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20"
          >
            <span>Next: Prajakta’s Section</span>
